...
proc removeOldLaunchers*() =
  #[
    Remove all old launchers that isn't in Denios's launchers db anymore
  ]#
  for kind, path in walkDir(dirLaucherDest):
    ...
    # Check if the launcher is Denios's specific
    if (currentLauncher.startsWith("denios-") or currentLauncher.startsWith("serv-")) and currentLauncher.endsWith(".desktop"):
      # Get package name from launcher. If package name != "" then it belongs to denios-menu (or old one)
