...
proc getXPackageName(path: string): string =
  for line in lines(path):
    # Normaly it starts with X-Denios-Packages
    # But some launchers might contains X-Denios-package (typo mistake)
    # So we use keyword X-Denios only
    if line.startsWith("X-Denios-"):
      return line.split("=")[1]
  return ""
...
proc fixOldLaunchers(path: string) =
  #[
    Some new launchers have been changed name to denios-* to serv-*
  ...
  for checkName in newNameArr:
    ...
    if fileName.startsWith(checkName):
      isDeleteNeeded = true
      destName &= "denios-" & fileName.replace(checkName, "")
      break
...
proc update_launchers*() =
  ...
  for kind, path in walkDir(dirLauncherSource):
    ...
    if fileName.startsWith("denios-") or fileName.startsWith("serv-"):
      let
        aptDeniosPackage = getXPackageName(path)
      if aptDeniosPackage != "":
        #[
        ...
        try:
          ...
          if installed.contains(aptDeniosPackage):
